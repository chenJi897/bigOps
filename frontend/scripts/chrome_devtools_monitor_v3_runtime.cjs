...
async function connectCDP() {
  const chrome = spawn('/opt/google/chrome/chrome', [
    ...
    '--user-data-dir=/tmp/cdt-profile-monitor-v3-runtime',
    'about:blank',
  ], { stdio: 'ignore' });
...
(async () => {
  ...
  const checks = [];
  const targets = [
    ['/monitor/dashboard', '按服务树聚合'],
    ['/monitor/alert-rules', '告警静默'],
    ['/monitor/silences', '告警静默'],
    ['/monitor/oncall', 'OnCall 值班'],
  ];

  for (const [path, text] of targets) {
    await send('Page.navigate', { url: `http://127.0.0.1:5173${path}` }, sessionId);
    await sleep(2200);
    const href = await evalJS(send, sessionId, 'location.href');
    const body = await evalJS(send, sessionId, 'document.body.innerText');
    checks.push({ path, href, ok: body.includes(text) });
  }

  const capture = await send('Page.captureScreenshot', { format: 'png' }, sessionId);
  fs.writeFileSync('/tmp/chrome-devtools-monitor-v3-runtime.png', Buffer.from(capture.data, 'base64'));
  const result = { checks, ...drainEvents(events, sessionId) };
  fs.writeFileSync('/tmp/chrome-devtools-monitor-v3-runtime.json', JSON.stringify(result, null, 2));
