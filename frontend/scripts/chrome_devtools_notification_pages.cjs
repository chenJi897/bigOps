...
async function connectCDP() {
  const chrome = spawn('/opt/google/chrome/chrome', [
    ...
    '--user-data-dir=/tmp/cdt-profile-notify-pages',
    'about:blank',
  ], { stdio: 'ignore' });
...
(async () => {
  ...
  const targets = [
    ['/notification/console', '渠道配置'],
    ['/notification/preferences', '我的通知设置'],
  ];
  ...
  fs.writeFileSync('/tmp/chrome-devtools-notification-pages.png', Buffer.from(capture.data, 'base64'));
  const result = { checks, ...drainEvents(events, sessionId) };
  fs.writeFileSync('/tmp/chrome-devtools-notification-pages.json', JSON.stringify(result, null, 2));
