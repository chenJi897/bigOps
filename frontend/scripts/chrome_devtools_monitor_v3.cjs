...
async function connectCDP() {
  const chrome = spawn('/opt/google/chrome/chrome', [
    ...
    '--user-data-dir=/tmp/cdt-profile-monitor-v3',
    'about:blank',
  ], { stdio: 'ignore' });
...
(async () => {
  ...
  const checks = [];
  const targets = [
    ['/monitor/datasources', '监控数据源'],
    ['/monitor/query', 'PromQL 查询台'],
    ['/monitor/alerts', '告警事件中心'],
    ['/monitor/agents/81fa4fbb-8a2b-4938-a368-e46ff57679c5', 'Agent 详情'],
  ];
  ...
  fs.writeFileSync('/tmp/chrome-devtools-monitor-v3.png', Buffer.from(capture.data, 'base64'));
  ...
  fs.writeFileSync('/tmp/chrome-devtools-monitor-v3.json', JSON.stringify(result, null, 2));
