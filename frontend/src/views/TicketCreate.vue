...
function selectType(t: any) {
  ...
  form.value = {
    ...
    ticket_kind: 'incident',
    priority: t.priority || 'medium',
    handle_dept_id: t.handle_dept_id || undefined,
  }
  selectedAssets.value = []
  draftSelectedAssets.value = {}
  draftSelectedAssetIDs.value = []
  step.value = 2
}
...
async function submitForm() {
  ...
  if (!form.value.type_id && !form.value.request_template_id) {
    ElMessage.warning('请选择工单类型或请求模板')
    return
  }
...
<template>
  <div class="page">
    <el-card shadow="never">
      ...
      <!-- Step 1: 选择类型 -->
      <div v-if="step === 1">
        <p style="margin-bottom: 16px; color: #606266;">请选择工单类型或请求模板：</p>
        <el-row :gutter="16">
            ...
            <el-card shadow="hover" class="type-card request-card" @click="selectRequestTemplate(template)" style="cursor: pointer; margin-bottom: 16px;">
              <div style="display: flex; align-items: center; gap: 12px;">
                ...
                <div>
                  ...
                  <div style="font-size: 12px; color: #909399;">{{ template.description || template.code }}</div>
                </div>
              </div>
            </el-card>
          </el-col>
        </el-row>
        <el-empty v-if="allTypes.length === 0 && allRequestTemplates.length === 0" description="暂无工单类型或请求模板，请先创建" />
      </div>

      <!-- Step 2: 填写表单 -->
      <div v-if="step === 2">
        <el-button link @click="step = 1" style="margin-bottom: 16px;"><el-icon><ArrowLeft /></el-icon> 返回选择类型</el-button>
        <el-tag style="margin-left: 8px;">{{ selectedType?.name || selectedTemplate?.name }}</el-tag>
        <el-tag v-if="selectedTemplate" type="success" style="margin-left: 8px;">请求模板</el-tag>
