...
async function loadConfig() {
  ...
  try {
    ...
    configForm.value = {
      ...
      webhook: {
        enabled: res.data?.webhook?.enabled || false,
        url: res.data?.webhook?.url || '',
        secret: res.data?.webhook?.secret || '',
        timeout_seconds: res.data?.webhook?.timeout_seconds ?? 10,
      },
      email: {
        enabled: res.data?.email?.enabled || false,
        host: res.data?.email?.host || '',
        port: res.data?.email?.port ?? 25,
        username: res.data?.email?.username || '',
        password: res.data?.email?.password || '',
        from: res.data?.email?.from || '',
      },
      message_pusher: {
        ...
        channel: res.data?.message_pusher?.channel || '',
...
<template>
  <div class="page">
    <el-card shadow="never">
      <template #header><span>通知配置中心</span></template>

      <el-alert
        title="这里负责全局通知渠道配置与投递验证。业务规则页只负责选择用哪些渠道。"
        type="info"
        show-icon
        :closable="false"
      ...
      <el-form :model="configForm" label-width="130px" style="max-width: 860px;" v-loading="configLoading">
        <el-form-item label="默认渠道">
          <el-select v-model="configForm.default_channels" multiple clearable style="width: 100%;">
            <el-option v-for="item in channelOptions" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="最大重试">
          <el-input-number v-model="configForm.max_retries" :min="0" :max="20" />
        </el-form-item>
        <el-form-item label="重试间隔(秒)">
          <el-input-number v-model="configForm.retry_interval_seconds" :min="1" :max="3600" />
        </el-form-item>
        <el-form-item label="扫描间隔(秒)">
          <el-input-number v-model="configForm.retry_scan_interval_seconds" :min="1" :max="3600" />
        </el-form-item>

        <el-divider>Webhook</el-divider>
        <el-form-item label="启用 Webhook">
          <el-switch v-model="configForm.webhook.enabled" />
        </el-form-item>
        <el-form-item label="Webhook URL">
          <el-input v-model="configForm.webhook.url" placeholder="https://example.com/hook" />
        </el-form-item>
        <el-form-item label="Webhook Secret">
          <el-input v-model="configForm.webhook.secret" placeholder="可选，用于签名" />
        </el-form-item>

        <el-divider>Email</el-divider>
        <el-form-item label="启用 Email">
          <el-switch v-model="configForm.email.enabled" />
        </el-form-item>
        <el-form-item label="SMTP Host">
          <el-input v-model="configForm.email.host" placeholder="smtp.example.com" />
        </el-form-item>
        <el-form-item label="SMTP Port">
          <el-input-number v-model="configForm.email.port" :min="1" :max="65535" />
        </el-form-item>
        <el-form-item label="用户名">
          <el-input v-model="configForm.email.username" />
        </el-form-item>
        <el-form-item label="密码">
          <el-input v-model="configForm.email.password" type="password" show-password />
        </el-form-item>
        <el-form-item label="发件地址">
          <el-input v-model="configForm.email.from" placeholder="bot@example.com" />
        </el-form-item>

        <el-divider>Message Pusher</el-divider>
        <el-form-item label="启用 Message Pusher">
          <el-switch v-model="configForm.message_pusher.enabled" />
        </el-form-item>
        <el-form-item label="服务地址">
          <el-input v-model="configForm.message_pusher.server" placeholder="http://message-pusher:8080" />
        </el-form-item>
        <el-form-item label="用户名">
          <el-input v-model="configForm.message_pusher.username" />
        </el-form-item>
        <el-form-item label="Token">
          <el-input v-model="configForm.message_pusher.token" type="password" show-password />
        </el-form-item>
        <el-form-item label="默认频道">
          <el-input v-model="configForm.message_pusher.channel" placeholder="可选" />
        </el-form-item>
        <el-form-item label="超时(秒)">
          <el-input-number v-model="configForm.message_pusher.timeout_seconds" :min="1" :max="120" />
        </el-form-item>
        <el-form-item>
          <el-button type="primary" :loading="configSaving" @click="saveConfig">保存通知配置</el-button>
        </el-form-item>
      </el-form>
