...
func (s *RequestTemplateService) Create(item *model.RequestTemplate) error {
	...
	item.NotifyChannels = normalizeTemplateChannels(item.NotifyChannels)
	if item.TicketKind == "" {
...
func (s *RequestTemplateService) Update(id int64, item *model.RequestTemplate) error {
	...
	existing.NotifyChannels = item.NotifyChannels
...
func normalizeTemplateChannels(raw string) string {
