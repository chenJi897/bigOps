...
func (s *ApprovalService) act(instanceID, approverID int64, action, comment string) error {
	...
	err := database.GetDB().Transaction(func(tx *gorm.DB) error {
		...
		stage := findStageByNo(policy.Stages, instance.CurrentStageNo)
		if stage == nil {
		...
		currentStageRecords, err := s.instanceRepo.ListRecordsByInstanceStage(instance.ID, instance.CurrentStageNo)
		if err != nil {
			return err
		}
		...
		nextStage := findNextStage(policy.Stages, instance.CurrentStageNo)
		if nextStage == nil {
