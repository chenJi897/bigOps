...
func (r *NotificationRepository) MarkAllReadByUserID(userID int64, readAt model.LocalTime) error {
	return database.GetDB().Model(&model.InAppNotification{}).
		Where("user_id = ? AND read_at IS NULL", userID).
		Update("read_at", readAt).Error
}

func (r *NotificationRepository) ClearReadByUserID(userID int64) (int64, error) {
	result := database.GetDB().
		Where("user_id = ? AND read_at IS NOT NULL", userID).
		Delete(&model.InAppNotification{})
