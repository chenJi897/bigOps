...
| Casbin 白名单边界 | 为了页面下拉方便而把整类业务 GET 接口放入白名单，会让“已登录但无菜单权限”的用户直接读到监控/任务/CI-CD 等业务数据；正确做法应是仅放公共轻量接口，不放整类业务读接口 |
| CI/CD 状态默认值语义 | `status=0` 在创建和更新语义不同：创建时 0 往往代表未显式设置，应回退成启用；更新时 0 代表显式禁用，不能统一强行改成 1 |
| Agent 任务回传边界 | `ReportOutput` 开流失败或执行 panic 如果只打日志不回状态，服务端会出现执行记录卡住；最少应做开流重试、panic recovery 和任务 goroutine 收口 |
